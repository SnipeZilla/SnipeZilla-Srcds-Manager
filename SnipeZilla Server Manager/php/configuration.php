...
$total = sizeof($server);
$output = shell_exec("powershell \"ipconfig | Select-String -Pattern 'IPv4 Address' | ForEach-Object { (\$_.ToString().Split(':')[1]).Trim() }\"");
$IPAddress = array_filter(explode("\n", $output));
...
<table>
	...
	<div id="server-0" class="setup">
			<div title="SteamCMD.exe location">
				<span class="title">SteamCMD Location</span><span name="steamcmddir" class="dir path steamcmd"><?php echo $server[0]['steamcmddir'];?></span><span class="fa fa-search browse" name="dir" data-app="steamcmd" title="Select steamcmd.exe"></span>
			</div>

			<div title="steam.exe path location. For CS2! dll dependency.">
				<span class="title">Path for Steam.exe</span><span name="steamdir" class="dir path steam"><?php echo $server[0]['steamdir'];?></span><span class="fa fa-search browse" name="dir" data-app="steam" title="Select steam.exe"></span>
			</div>
			...
			<div title="SMTP port. Default is 25, no ssl">
				<span class="title" title="usually 25 or 587 for SSL">SMTP port</span><input class="small" type="text" name="smtp_port" placeholder="25" value="<?php echo $server[0]['email']['smtp_port'];?>">
	...
	?>
			<div title="Srcds Game">
				<span class="title">Game</span>
					<select name="appid">
						<option value="0" <?php if ($server[$i]['pid']) echo ' disabled="disabled"';?>>--Select a game--</option>
						<?php
						foreach ($games as $game=>$k) {
                            $class  = $games[$game]['login']?"login ":'';
                            $class .= $games[$game]['metamod']?"metamod ":'';
                            $class .= $games[$game]['rcon_password']?"rcon_password ":'';
							echo "<option ".($class?'class="'.$class.'" ':'')."value=\"".$game."\"".(isset($server[$i]['appid']) && $server[$i]['appid']==$game?' selected="selected"':($server[$i]['pid']?' disabled="disabled"':'')).">".$games[$game]['name']." [".$game."]</option>";
						}
					?>
					</select>
			...
			<div title="Path to the root server directory.">
				<span class="title">Install Dir</span><span class="dir path steamcmd" name="installdir"><?php echo $server[$i]['installdir'];?></span>
				<?php if (!$server[$i]['pid']) echo '<span class="fa fa-search browse" name="dir" data-app="steamcmd" title="Select Root Directory"></span>';?>
			</div>
			<div title="Path to the Server Executable(e.g. srcds.exe).">
				<span class="title">SRCDS.exe</span><span class="dir file" name="srcds"><?php echo $server[$i]['srcds'];?></span>
				<?php if (!$server[$i]['pid']) echo '<span class="fa fa-search browse" name="file" title="Select server app"></span>';?>
			</div>
			<div title="Server IP">
			...
			<div title="Your STEAM username <username> <password>(optional), leave empty for 'anonymous'(Will not work)." class="steamlogin">
				<span class="title">Login</span><input type="text" name="login" value="<?php echo (isset($server[$i]['login'])?$server[$i]['login']:"");?>">
			</div>
			<div title="Srcds Process Priority.">
				<span class="title">Srcds Priority</span>
					<select name="priority">
			...
			<div title="Start parameters for srcds.exe (e.g. -nohltv -maxplayers_override 32 +sv_lan 1 +map de_dust2)">
				<span class="title">Additionnal Parameters:</span><br/><textarea rows="2" cols="50" name="cmd"><?php echo (isset($server[$i]['cmd'])?$server[$i]['cmd']:"");?></textarea>
			</div>
			...
			<div class="rcon_password" title="RCON password. if not present in server.cfg like CS2">
				<span class="title">RCON password</span><input type="text" name="rcon_password" value="<?php echo $server[$i]['rcon_password'];?>">
			</div>
            <div class="metamod" title="Add metamod to gameinfo.gi. Prevent overwrite after update">
			    <span class="title">METAMOD</span><input type="checkbox" name="metamod" value="1" <?php if ($server[$i]['metamod']=='true') {echo ' checked';}?>>
			</div>
        <?php
...
//----------------
    var games={};
    games=<?php echo json_encode($games);?>;
//--------------------------
//         TABS
//--------------------------
    ...
    $("#add-server").click(function() {
        ...
        $('#server-'+ Sz.tab.id +' .path').text('');
        $('#server-'+ Sz.tab.id +' .file').text('');
        ...
        var p=$('#server-0 [name="priority"] option:selected').val();
        $('#server-'+(Sz.tab.id)+' [name="priority"]').val(p);
        //Refresh Tabs:
        reTabs();
    }); 
    ...
    function reTabs() {
        ...
        $( ".browse" ).click(function() {
            Sz.name=$(this).attr("name");
            Sz.app=$(this).attr("data-app");
            drive.dialog( "open" );
        });
        ...
        $( ".update" ).click( function() {
            ...
            Sz.Ajax('php/setup.php',{fct:'update', par: Sz.tab.id, token: Sz.token, xcopy: $('#server-0 [name="xcopy"]').prop('checked')||$('#server-'+Sz.tab.id+' [name="xcopy"]').prop('checked') },'#saved');
        ...
        $('[name="appid"]').change(function() {
            ...
            if ($(this).find('option:selected').hasClass('metamod')) {
                $('div.metamod:eq('+(Sz.tab.id-1)+')').show();
            } else {
                $('div.metamod:eq('+(Sz.tab.id-1)+')').hide();
            }
            if ($(this).find('option:selected').hasClass('rcon_password')) {
                $('div.rcon_password:eq('+(Sz.tab.id-1)+')').show();
            } else {
                $('div.rcon_password:eq('+(Sz.tab.id-1)+')').hide();
            }
            $('#server-'+(Sz.tab.id)+' .suggest-map').text('').removeClass('hovered').hide();
            if ( $('#server-'+Sz.tab.id+' [name="appid"] option:selected').val() > 0 ) { 
                $('#server-'+(Sz.tab.id)+' .browse').show();
            } else { $('#server-'+(Sz.tab.id)+' .browse').hide(); }
        });
        ...
        for (var i=0;i<num_appid;i++){
            ...
            if ($('[name="appid"]:eq('+i+')').find('option:selected').hasClass('metamod')) {
                $('div.metamod:eq('+(i)+')').show();
            } else {
                $('div.metamod:eq('+(i)+')').hide();
            }
            if ($('[name="appid"]:eq('+i+')').find('option:selected').hasClass('rcon_password')) {
                $('div.rcon_password:eq('+(i)+')').show();
            } else {
                $('div.rcon_password:eq('+(i)+')').hide();
            }
        }
...
//--------------------------
    ...
    function Explorer() {
        ...
        $.ajax({
            ...
            data: {steam: Sz.tab.id, token: Sz.token, drive:'<?php if ( $server[1]['installdir'] ) { echo substr($server[1]['installdir'],0,1); } else { echo substr($root,0,1); } ?>'},
            cache: false,
            async: true
        }).done(function(data) {
            $('#hdd').show().html(data);
    ...
    function ValidPath() {
        ...
        if (Sz.tab.id == 0) {
            const re = new RegExp(Sz.app+".exe$", "i")
            if (!str.match(re)) {
                updateTips( 'Require '+Sz.app+'.exe' );
                return false;
            }
            if ( Sz.app == 'steam' ) str=str.replace("steam.exe","");
            $('.'+Sz.app+':eq('+Sz.tab.id+')').text(str);
            ...
            if (Sz.name=="dir") {
                ...
                if ( Sz.app == 'steamcmd' ) {
                    var appid=$('#server-'+Sz.tab.id+' [name="appid"] option:selected').val();
                    if ( appid > 0 ) {
                        var app=$('.file:eq('+(Sz.tab.id-1)+')').html() || games[appid]['srcds'];
                        if ( !/^\w:/.test(app) ) {
                            $('.file:eq('+(Sz.tab.id-1)+')').text(str+'\\'+app);
                        }
                    }
                }
            }
            if (Sz.name=="file") {
                if (!str || !str.match(/:\\/)) {
                    updateTips( 'Select an .exe' );
                    //return false;
                }
                $('.file:eq('+(Sz.tab.id-1)+')').text(str);
            }
        }
        drive.dialog( "close" );
...
//--------------------------
     function getConfig() {
        jsonObj = [];
            ...
            Cfg ["steamcmd"]["steamdir"] = $('#server-0 [name="steamdir"]').html();
            ...
            if ($('div#server ul li:eq(1)').is(':visible') ) {
                ...
                $('.server-sz').each(function(i, obj) {
                    ...
                    Cfg ["server"][i] = {
										...
										srcds: $('#server-'+y+' [name="srcds"]').html(),
                    ...
                    if ( $('#server-'+y+' [name="metamod"]').is(":visible") )
                    Cfg ["server"][i]['metamod'] = $('#server-'+y+' [name="metamod"]').prop('checked');

                    if ($('#server-'+y+' [name="rcon_password"]').val().trim() && $('#server-'+y+' [name="rcon_password"]').is(":visible") )
                    Cfg ["server"][i]['rcon_password'] = $('#server-'+y+' [name="rcon_password"]').val();
