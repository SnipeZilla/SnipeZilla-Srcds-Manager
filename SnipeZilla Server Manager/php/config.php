...
//File
$root = exec('chdir',$o,$r);
$file = preg_replace('/\\\\web.*+$/','\config.xml',$root);
$file_steamcmd = preg_replace('/\\\\web.*+$/','\steamcmd\steamcmd.exe',$root);
...
if ( !$xml ) {
        ...
        $server[0] = $steamcmd;
        $server[0]['steamcmddir'] = file_exists($file_steamcmd)?$file_steamcmd:'';
        $server[0]['steamdir'] = '';
...
if ( !isset($server[0]['steamdir']) ) {

    $server[0]['steamdir']="";

}

$server[0]['steamdir'] = preg_replace('/\/$|\\$/', '', str_replace("/","\\",$server[0]['steamdir']));

if ( !file_exists($server[0]['steamdir']) ) {
...
foreach($xml->children() as $child) {
    ...
    if ( $role && preg_match("/[A-Za-z0-9]+/", $role) ) {
        ...
        if ( !isset($server[$id]['srcds']) ) {
            $server[$id]['srcds']=$server[$id]['installdir'].'\\'.$games[$role]['srcds'];
        }
        $server[$id]['app'] = substr($server[$id]['srcds'],strrpos($server[$id]['srcds'], '\\')+1);
        ...
        //rcon
        if ( !isset($server[$id]['metamod']) ) {
   
                $server[$id]['metamod'] = '';
       
        }
        //rcon
        if ( !isset($server[$id]['rcon_password']) ) {
   
                $server[$id]['rcon_password'] = '';
        ...
        //Running?
        $server[$id]['pid'] = processID($server[$id]['app'], $server[$id]['srcds'], $server[$id]['port']);
