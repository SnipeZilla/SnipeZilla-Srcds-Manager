...
//Require Attr
$attr  = array('name','game','server','login','cmd','app_set_config','config','srcds','metamod','rcon_password');
$games = [];
$game  = [];
foreach($xml->children() as $child) {
    ...
    if ( $role && preg_match("/[A-Za-z0-9]+/", substr( $role ,0,1) ) ) {
        ...
        if (  isset($game[$role]['name']) && isset($game[$role]['game']) && isset($game[$role]['server']) ) {
            ...
            if (!isset($games[$role]['metamod']) || $games[$role]['metamod'] != 'true') {
                $games[$role]['metamod'] = false;
            } else {
                $games[$role]['metamod'] = true;
            }

            if (!isset($games[$role]['rcon_password']) || $games[$role]['rcon_password'] != 'true') {
                $games[$role]['rcon_password'] = false;
            } else {
                $games[$role]['rcon_password'] = true;
            }
