...
function QueryTask($SzMgr) {
    ...
    if ( preg_match('/'.$SzMgr.'/i', shell_exec('schtasks /Query')) ) {
        ...
        $cmd = preg_split('/commandLine=/i', shell_exec('wmic process where name="php.exe" get commandLine /FORMAT:LIST'),-1, PREG_SPLIT_NO_EMPTY);
...
function KScript() {
        ...
        $cmd = preg_split('/\w+=/', shell_exec('wmic process where name="php.exe" get commandLine,processId /FORMAT:LIST'),-1, PREG_SPLIT_NO_EMPTY);
        ...
        for ($i=2; $i<$numb; $i++) {
            ...
            if ( preg_match('/.*bin\\\\SzMgr.php"?/i', $cmd[$i]) ) {
                ...
                if ((int)$cmd[$i+1]) shell_exec("taskkill /F /PID ".$cmd[$i+1].""); 
                break;
...
switch ($task) {
    ...
    case 'create' :
        ...
        if (file_exists($path)) {
            ...
            $query[0] = 'Create';
            $query[1] = ' [Could not create Task, permission denied.]';
        }
