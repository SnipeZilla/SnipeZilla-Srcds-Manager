...
<table>
	...
	<div id="server-0" class="setup">
			<div title="SteamCMD.exe location">
				<span class="title">SteamCMD Location</span><span name="steamcmddir" class="dir path"><?php echo $server[0]['steamcmddir'];?></span><span class="fa fa-search browse" name="dir" title="Select steamcmd.exe"></span>
			</div>
	...
	?>
				...
				<span class="title">Game</span>
					<select name="appid">
						...
						foreach ($games as $game=>$k) {
							echo "<option ".($games[$game]['login']?'class="login" ':'')."value=\"".$game."\"".(isset($server[$i]['appid']) && $server[$i]['appid']==$game?' selected="selected"':($server[$i]['pid']?' disabled="disabled"':'')).">".$games[$game]['name']." [".$game."]</option>";
						}
					?>
					</select>
			...
			<div title="Path to the root server directory.">
				...
				<?php if (!$server[$i]['pid']) echo '<span class="fa fa-search browse" name="dir" title="Select Root Directory"></span>';?>
			</div>
