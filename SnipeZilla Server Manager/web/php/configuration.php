...
//--------------------------
    ...
    function Explorer() {
        ...
        $.ajax({
            ...
            data: {steam: Sz.tab.id, token: Sz.token, drive:'<?php if ( $server[1]['installdir'] ) echo substr($server[1]['installdir'],0,1); ?>'},
            cache: false,
            async: true
        }).done(function(data) {
            $('#hdd').show().html(data);
    ...
    function ValidPath() {
        ...
        if (Sz.tab.id == 0) {
            ...
            if (Sz.name=="dir") {
                ...
                var appid=$('#server-'+Sz.tab.id+' [name="appid"] option:selected').val();console.log(appid);
                if ( appid > 0 ) {
