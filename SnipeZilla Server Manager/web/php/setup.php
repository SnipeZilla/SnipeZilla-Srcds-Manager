...
switch ($fct) {
    ...
    case 'update':
        ...
        for ($i=1; $i<$numb; $i++) {
            ...
            if ( preg_match('/.*bin\\\\xcopy.php"?/i', $proc[$i]) && $copy !== 'false' ) {
        ...
        $steamcmd = $server[0]['steamcmddir'].' +force_install_dir \"'.$server[$par]['installdir'].'\"'.' +login '.$server[$par]['login'].' '.$games[$server[$par]['appid']]['app_set_config'].' +app_update '.$games[$server[$par]['appid']]['server'].' validate +quit';
