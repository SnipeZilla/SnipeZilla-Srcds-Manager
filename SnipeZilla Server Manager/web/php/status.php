...
if ($task_status != 'Running') {
    echo '<p><span class="fa fa-exclamation-triangle"></span>&nbsp;';
    if ( $task_status=='Create' ) {
        if ( $app_status=='ON' ) {
            echo 'SnipeZilla Srcds Manager is not installed but running.';
        } else {
                ...
                if (!info.error) {
                    ...
                    if (info.stop) {
                        ...
                        if ( info.pid ) {
                            ...
                            setTimeout( Refresh, 2500,ip,port,id);
                        } else { 
    ...
    function Ctrl(v) {
        ...
        $.ajax({
            ...
            try{
                ...
                if ( !$('#a2s-info #svr-'+id).find('.players').text().trim() && $('#a2s-info #svr-'+id).find('.fa').hasClass('fa-stop') ) {
                    setTimeout( Refresh, 2500,ip,port,id);
                }
            }catch(err){
                alert(data);
