...
for ($i=0; $i<$total; $i++) {
    ...
    for ($ii=0; $ii<$i; $ii++) {
        ...
        if ( $str['server'][$i]['installdir'] == $str['server'][$ii]['installdir'] && $str['server'][$i]['ip'].':'.$str['server'][$i]['port'] == $str['server'][$ii]['ip'].':'.$str['server'][$ii]['port'] ) {

            Error('Error->server #'.($i+1).': Install Dir and port already selected in server #'.($ii+1));
