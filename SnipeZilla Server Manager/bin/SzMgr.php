...
//-------------------------------------------
    private function Email()
    {
        ...
        //Sendmail path
        $sendmail = $this->__DIR.'\bin\php.exe -dextension=\"'.$this->__DIR.'\bin\php_openssl.dll\" \"'.$this->__DIR.'\bin\sendmail.php\" \"'.$this->server[0]['email']['mail'].'\" \"'.base64_encode($message).'\" ';
...
//-------------------------------------------
    private function GetConfig()
    {
        ...
        if (isset($this->server[0]['email']['sendmail_to']) && empty($this->server[0]['email']['sendmail_to']) ) {
            ...
            //No email
            $this->server[0]['email'] = array('mail' => '', 'alert' => '', 'postpone' => array(time(),30,0),'message' => '');
...
//-------------------------------------------
    private function Update()
    {
        ...
        if ( $xcopy && $ready ) {
            ...
            $steamcmd = $this->server[0]['steamcmddir'].' +login '.$this->server[$this->id]['login'].' +force_install_dir \"'.$path.'\"'.$this->games[$this->server[$this->id]['appid']]['app_set_config'].' +app_update '.$this->games[$this->server[$this->id]['appid']]['server'].' validate +quit';
